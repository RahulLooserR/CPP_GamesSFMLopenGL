...
void init()
{
	...
	msgFont.loadFromFile("Assets/fonts/arial.ttf");
	...
	scoreFont.loadFromFile("Assets/fonts/waltographUI.ttf");
	...
	// rocket font
	rocketFont.loadFromFile("Assets/fonts/waltographUI.ttf");
	...
	hero.init("Assets/graphics/heroAnim.png", sf::Vector2f(viewSize.x * 0.25f, viewSize.y * 0.5f), 200, 4, 1.0f);
...
void init_window()
{
	...
	window.create(vm, "Bazooka Girl", sf::Style::Default);
}
...
void shoot()
{
	if (rocketLeft > 0)
	{
		...
		rocket->init("Assets/graphics/rocket.png", hero.getSprite().getPosition(), 500.0f);
